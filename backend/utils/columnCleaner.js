...
const forwardFillHeaderRow = (values) => {
  const filled = [];
  let lastValue = "";
...
const removeDateTokens = (value) => {
  ...
  tokens.forEach((token, index) => {
    ...
    if (token && /^\d+$/.test(token) && index < tokens.length - 1) return;
...
const finalizeColumnNamesWithOffset = (columns, duplicateSuffixOffset = 1) => {
  const seen = new Map();
  const finalCols = [];
...
const finalizeColumnNames = (columns) => finalizeColumnNamesWithOffset(columns, 1);

const stripTrailingDigitsFromToken = (token) => {
...
const addComparisonVariant = (variants, tokens) => {
...
const getComparisonKeyVariants = (value) => {
...
const isNumericSectionHeader = (value) => {
  const trimmed = String(value || "").trim();
  return Boolean(trimmed && /^\d+(\.\d+)?$/.test(trimmed));
};

const buildHierarchicalColumnNames = (parentRow, childRow) => {
  ...
  for (let index = 0; index < maxLength; index += 1) {
    ...
    // Skip purely numeric section headers (e.g. "2", "3") so they don't
    // pollute column names like "2_gsr_q_wood"
    if (parentCell && !isNumericSectionHeader(parentCell)) {
  ...
  return finalizeColumnNamesWithOffset(merged, 1);
};

module.exports = {
  buildHierarchicalColumnNames,
  cleanColumnName,
  forwardFillHeaderRow,
